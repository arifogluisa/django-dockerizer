...
def parse_arguments():
    ...
    parser.add_argument("--celery", help="Include Celery configurations", action="store_true")
...
def generate_docker_files(env_type):
    ...
    if args.celery and env_type == 'prod':
        with open(os.path.join(BASE_DIR, 'bin', env_type, 'redis.dockerfile'), 'w') as file:
...
def generate_single_files():
    ...
    for file_name, content in SINGLE_FILES:
        with open(os.path.join(BASE_DIR, 'bin', file_name), 'w') as file:
            file.write(content)
...
def generate_docker_compose_files(env_type):
    if args.celery:
        content = DOCKER_COMPOSE_WITH_CELERY_DEV if env_type == 'dev' else DOCKER_COMPOSE_WITH_CELERY_PROD
    else:
        content = DOCKER_COMPOSE_DEV if env_type == 'dev' else DOCKER_COMPOSE_PROD
    with open(os.path.join(BASE_DIR, 'bin', env_type, 'docker-compose.yml'), 'w') as file:
        file.write(content)
